...
Pu_db = -20;
Pu = 10.^(Pu_db./10);
P0 = 6;            % Transmit Power of the HAP (in W)
...
num_runs = 10;
num_frames = 5000;             % Total number of frames
num_devices = 1:10;               % Number of users
                ...
                for run = 1:num_runs(i0)
                    ...
                    for j = 1:num_Qframes_IL
                        ...
                        for i = 1:num_devices(i3)
                            ...
                            battery_level = state(i) + (g(P(i))*tau_C(i4)) - (sendPkt)*pTransmit*(abs(hi)^2)/H^n;
                            ...
                            battery_level = state(i) + (g(P(i))*tau_C(i4)) - ((rho(i))*pTransmit*(abs(hi)^2)/H^n); % (!) State in t, battery level for t+1
                            battery_level(battery_level<0) = nan;
...
AIDrwrd_POTfull = rwrd_POT;


%% No sending replicas
fprintf('AVG. CSI - A =2 M = 4\n') 
...
% plot(P0,AIDrwrd_POTfull,'--s','Color',[0 0.4470 0.7410],'LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w')
% hold on
% plot(P0,AIDrwrd_POTavg,'--s','Color',[0.8500 0.3250 0.0980],'LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w')
% hold off
% grid on;
% axis([0.5 8 0.3 2.7])
% ax = gca;
...
% legend('F-CSI, M = 4','A-CSI, M = 4','F-CSI, M = 8','A-CSI, M = 8', 'F-CSI, M = 4 [CRDSA]','A-CSI, M = 4 [CRDSA]','FontSize', 10); 
...
plot(reshape(AIDrwrd_POTfull,[1,length(num_devices)]),'--s','Color',[0 0.4470 0.7410],'LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w')
plot(reshape(AIDrwrd_POTavg,[1,length(num_devices)]),'--s','Color',[0.8500 0.3250 0.0980],'LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w')
hold off
grid on;
axis([1 10 0 2.4])
ax = gca;
...
% plot(kpb_db ,reshape(AIDrwrd_POTfull,[1,length(kpb_db)]),'--s','Color',[0 0.4470 0.7410],'LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w')
% plot(kpb_db ,reshape(AIDrwrd_POTavg,[1,length(kpb_db)]),'--s','Color',[0.8500 0.3250 0.0980],'LineWidth',2,'MarkerSize',10,'MarkerFaceColor','w')
% hold off
% grid on;
% % axis([1 10 0 2.7])
% ax = gca;
...
% ylabel('Average Number of Successful Transmissions per Frame','FontSize',10,'Interpreter','latex');
% legend('F-CSI, M = 4','A-CSI, M = 4', 'F-CSI, M = 4 [CRDSA]','A-CSI, M = 4 [CRDSA]','FontSize', 10); 
% 
